...
const YEAR_STATS = [
  { yr: 2021, cost: 2048, km: 22429, eff: 9.9, ppl: 1.322, fills: 54 },
  { yr: 2022, cost: 2655, km: 20766, eff: 10.8, ppl: 1.620, fills: 49 },
  { yr: 2023, cost: 2748, km: 21658, eff: 10.3, ppl: 1.424, fills: 40 },
  { yr: 2024, cost: 3614, km: 23126, eff: 10.0, ppl: 1.387, fills: 56 },
  { yr: 2025, cost: 3760, km: 22557, eff: 9.4, ppl: 1.223, fills: 47 },
  { yr: 2026, cost: 349,  km: 2333,  eff: 9.1, ppl: 1.136, fills: 9  },
];

const SparkLine = ({ data, valueKey, color }) => {
  const vals = data.map(d => d[valueKey]);
  const min = Math.min(...vals), max = Math.max(...vals), range = max - min || 1;
  const pts = vals.map((v, i) => `${(i / (vals.length - 1)) * 96 + 2},${36 - ((v - min) / range) * 30}`).join(' ');
  return (
    <svg viewBox="0 0 100 40" style={{ width: "100%", height: 40 }} preserveAspectRatio="none">
      <polyline points={pts} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.9" />
      {vals.map((v, i) => {
        const x = (i / (vals.length - 1)) * 96 + 2, y = 36 - ((v - min) / range) * 30;
  ...
  return (
    <svg viewBox="0 0 76 76" style={{ width: 76, height: 76 }}>
      ...
      {segments.map((seg, i) => {
        ...
        cum += pct;
        return <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={seg.color} strokeWidth="9"
...
<div style={{
  minHeight: "100vh",
  background: "linear-gradient(160deg, #080e1c 0%, #0c1424 60%, #080e1c 100%)",
...
<style>{`
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  textarea{resize:none;outline:none;border:none;background:transparent;color:#e2e8f0;font-family:inherit;font-size:14px;line-height:1.6;width:100%;}
  textarea::placeholder{color:#4a5568;}
  .chip:hover{background:rgba(56,189,248,0.14)!important;border-color:rgba(56,189,248,0.5)!important;transform:translateY(-1px);color:#7dd3fc!important;}
  .send-btn:hover{background:#0ea5e9!important;transform:scale(1.05);}
  .send-btn:disabled{opacity:0.3;cursor:not-allowed;}
  .stat-card{transition:transform 0.2s,box-shadow 0.2s;}
  .stat-card:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,0.5)!important;}
`}</style>

{/* ── TOP NAV ── */}
<div style={{
  background: "rgba(8,14,28,0.96)", backdropFilter: "blur(20px)",
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
    ...
    <div>
      <span style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#f1f5f9", letterSpacing: "-0.02em" }}>VehicleIQ</span>
      <span style={{ fontSize: 12, color: "#64748b", marginLeft: 10, letterSpacing: "0.05em", fontWeight: 500 }}>TOYOTA CAMRY 2014</span>
    </div>
  </div>
  <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
    <span style={{ fontSize: 13, color: "#94a3b8", fontWeight: 500 }}>S. M. Shaarif Anwar</span>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#22c55e", animation: "pulse 2s infinite" }} />
      <span style={{ fontSize: 12, color: "#94a3b8" }}>255 records · Feb 2021 – Mar 2026</span>
    </div>
  </div>
</div>
...
<div style={{
  ...
  padding: "16px 28px", display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12,
}}>
  {[
    { label: "Total Fuel Spend", value: "$15,174", sub: "5-year total", icon: "💰", color: "#fbbf24", sparkKey: "cost" },
    { label: "Distance Driven",  value: "112,869 km", sub: "since purchase", icon: "🛣️", color: "#38bdf8", sparkKey: "km" },
    { label: "Avg Efficiency",   value: "9.8 L/100km", sub: "vs 10.2 typical Camry", icon: "⛽", color: "#34d399", sparkKey: "eff" },
    { label: "Avg $/Litre",      value: "$1.377", sub: "all 255 fill-ups", icon: "📊", color: "#a78bfa", sparkKey: "ppl" },
    { label: "Total Litres",     value: "11,016 L", sub: "incl. gas can fills", icon: "🪣", color: "#fb923c", sparkKey: null },
  ].map((s, i) => (
    <div key={i} className="stat-card" style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        <span style={{ fontSize: 11, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", fontWeight: 600 }}>{s.label}</span>
        <span style={{ fontSize: 18 }}>{s.icon}</span>
      </div>
      <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 24, fontWeight: 800, color: "#f8fafc", lineHeight: 1.1, marginBottom: 4 }}>{s.value}</div>
      <div style={{ fontSize: 11, color: "#64748b", fontWeight: 500, marginBottom: s.sparkKey ? 8 : 0 }}>{s.sub}</div>
      {s.sparkKey && <div style={{ opacity: 0.9 }}><SparkLine data={YEAR_STATS.slice(0,5)} valueKey={s.sparkKey} color={s.color} /></div>}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${s.color}, transparent)` }} />
    </div>
  ))}
</div>
...
<div style={{
  ...
  display: "grid", gridTemplateColumns: "2.2fr 1.3fr 1fr", gap: 12,
}}>
  {/* Bar Chart */}
  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 18px" }}>
    <div style={{ fontSize: 12, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", fontWeight: 600, marginBottom: 14 }}>Annual Fuel Spend (CAD)</div>
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 70 }}>
      {YEAR_STATS.map((d, i) => {
        const maxCost = Math.max(...YEAR_STATS.map(x => x.cost));
        const h = (d.cost / maxCost) * 58;
        const colors = ["#60a5fa","#38bdf8","#34d399","#fbbf24","#f97316","#a78bfa"];
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            <div style={{ fontSize: 11, color: "#cbd5e1", fontWeight: 600 }}>${Math.round(d.cost/100)/10}k</div>
            <div style={{
              width: "100%", height: h, borderRadius: "4px 4px 0 0",
              background: colors[i], opacity: d.yr === 2026 ? 0.4 : 0.85,
            }} />
            <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>{`'${String(d.yr).slice(2)}`}</div>
          </div>
        );
      })}
    </div>
  ...
  {/* Efficiency Spark */}
  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 18px" }}>
    <div style={{ fontSize: 12, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", fontWeight: 600, marginBottom: 8 }}>Efficiency L/100km</div>
    <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 10 }}>
      <span style={{ fontFamily: "'Syne', sans-serif", fontSize: 26, fontWeight: 800, color: "#34d399" }}>↓ 8.1%</span>
      <span style={{ fontSize: 12, color: "#64748b" }}>vs 2022 worst</span>
    </div>
    <SparkLine data={YEAR_STATS.slice(0,5)} valueKey="eff" color="#34d399" />
    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
      {YEAR_STATS.slice(0,5).map(d => (
        <span key={d.yr} style={{ fontSize: 11, color: "#64748b", fontWeight: 500 }}>{`'${String(d.yr).slice(2)}`}</span>
      ))}
    </div>
  </div>

  {/* Donut */}
  <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14 }}>
    <DonutChart segments={[{ value: 15174, color: "#38bdf8" }, { value: 550, color: "#34d399" }, { value: 102, color: "#fbbf24" }]} />
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 12, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", fontWeight: 600, marginBottom: 10 }}>Cost Split</div>
      {[["Fuel", "$15,174", "#38bdf8"], ["Service", "~$550", "#34d399"], ["Washes", "$102", "#fbbf24"]].map(([lbl, val, col], i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: col, flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: "#94a3b8", flex: 1, fontWeight: 500 }}>{lbl}</span>
          <span style={{ fontSize: 13, color: "#f1f5f9", fontWeight: 700 }}>{val}</span>
        </div>
      ))}
    </div>
  </div>
...
{/* ── CHAT + SIDEBAR ── */}
<div style={{ flex: 1, display: "flex", overflow: "hidden", maxHeight: "calc(100vh - 310px)" }}>
  ...
  <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
      {messages.length === 0 && (
        <div style={{ animation: "fadeUp 0.5s ease both" }}>
          <div style={{
            ...
            border: "1px solid rgba(56,189,248,0.2)", borderRadius: 16, padding: "22px 26px", marginBottom: 18,
          }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 20, fontWeight: 800, color: "#f1f5f9", marginBottom: 8 }}>
              Ask me anything about your Camry
            </div>
            <div style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.7, maxWidth: 520, fontWeight: 500 }}>
              5 years of fuel data, maintenance history, and cost analytics — all queryable in plain English. Try a suggested query below or type your own.
            </div>
          </div>
          {showSuggestions && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {SUGGESTED_QUERIES.map((q, i) => (
                <button key={i} className="chip" onClick={() => sendMessage(q)} style={{
                  ...
                  borderRadius: 20, padding: "9px 15px", color: "#cbd5e1", fontSize: 12,
      ...
      {messages.map((msg, i) => (
        <div key={i} style={{
          ...
          marginBottom: 16, animation: "fadeUp 0.3s ease both",
        }}>
          {msg.role === "assistant" && (
            <div style={{
          ...
          <div style={{
            maxWidth: "76%",
            background: msg.role === "user" ? "linear-gradient(135deg, #0369a1, #075985)" : "rgba(255,255,255,0.05)",
            border: msg.role === "user" ? "1px solid rgba(56,189,248,0.35)" : "1px solid rgba(255,255,255,0.09)",
      ...
      {loading && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: "16px 16px 16px 4px", padding: "12px 16px" }}>
            <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
              {[0,1,2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#38bdf8", animation: "bounce 1.2s ease-in-out infinite", animationDelay: `${i*0.2}s` }} />)}
            </div>
          </div>
        </div>
      )}
    ...
    {/* INPUT */}
    <div style={{ padding: "12px 28px 18px", background: "rgba(8,14,28,0.97)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
      <div style={{
        ...
        <textarea ref={inputRef} rows={1} value={input}
          ...
          onKeyDown={handleKey}
          placeholder="Ask about fuel costs, efficiency trends, maintenance, comparisons..."
          style={{ flex: 1, minHeight: 22 }}
        />
        <button className="send-btn" onClick={() => sendMessage()} disabled={!input.trim() || loading}
          style={{ width: 36, height: 36, borderRadius: 10, border: "none", background: "#0ea5e9", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", flexShrink: 0 }}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>
          </svg>
        </button>
      </div>
      <div style={{ fontSize: 10, color: "#334155", textAlign: "center", marginTop: 8, letterSpacing: "0.07em" }}>
        VEHICLEIQ PROOF OF CONCEPT · CLAUDE AI · DATA: 2021–2026
      </div>
    </div>
  </div>
  ...
  {/* SIDEBAR */}
  <div style={{
    width: 230, flexShrink: 0, borderLeft: "1px solid rgba(255,255,255,0.07)",
    background: "rgba(255,255,255,0.015)", overflowY: "auto",
    padding: "18px 16px", display: "flex", flexDirection: "column", gap: 18,
  }}>
    {/* Year table */}
    <div>
      <div style={{ fontSize: 11, color: "#94a3b8", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>By Year</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, padding: "4px 0", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 4 }}>
          {["Year","Spend","L/100"].map(h => <span key={h} style={{ fontSize: 10, color: "#64748b", fontWeight: 700, letterSpacing: "0.06em" }}>{h}</span>)}
        </div>
        {YEAR_STATS.map(d => (
          <div key={d.yr} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
            <span style={{ fontSize: 13, color: "#e2e8f0", fontWeight: 600 }}>{d.yr === 2026 ? "2026*" : d.yr}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: d.cost > 3000 ? "#fbbf24" : "#94a3b8" }}>${d.cost >= 1000 ? (d.cost/1000).toFixed(1)+"k" : d.cost}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: d.eff < 9.5 ? "#34d399" : d.eff > 10.5 ? "#f87171" : "#94a3b8" }}>{d.eff}</span>
          </div>
        ))}
        <div style={{ fontSize: 10, color: "#475569", marginTop: 4, fontWeight: 500 }}>* Jan–Mar 2026 partial year</div>
      </div>
    </div>

    {/* Maintenance */}
    <div>
      <div style={{ fontSize: 11, color: "#94a3b8", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>Maintenance Status</div>
      {[
        { label: "Last Oil Change", val: "Jan 10, 2026", status: "good" },
        { label: "Current Odometer", val: "~129,329 km", status: "info" },
        { label: "Next Oil Due", val: "~137,500 km", status: "warn" },
        { label: "Tyre Set", val: "Winter (Nov '25)", status: "info" },
      ].map((item, i) => (
        <div key={i} style={{ padding: "8px 0", borderBottom: i < 3 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
          <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600, marginBottom: 3 }}>{item.label}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", flexShrink: 0, background: item.status === "good" ? "#22c55e" : item.status === "warn" ? "#f59e0b" : "#38bdf8" }} />
            <span style={{ fontSize: 13, color: "#f1f5f9", fontWeight: 600 }}>{item.val}</span>
          </div>
        </div>
      ))}
    </div>
    ...
    {/* Highlights */}
    <div>
      <div style={{ fontSize: 11, color: "#94a3b8", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 700, marginBottom: 12 }}>Highlights</div>
      {[
        ["✅", "Best efficiency", "2025 — 9.4 L/100km"],
        ["⚠️", "Worst efficiency", "2022 — 10.8 L/100km"],
        ["💚", "Cheapest fuel", "2026 — $1.14/L avg"],
        ["🔴", "Priciest fuel", "2022 — $1.62/L avg"],
        ["🏁", "Most KM driven", "2024 — 23,126 km"],
      ].map(([icon, lbl, val], i) => (
        <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 10 }}>
          <span style={{ fontSize: 14, flexShrink: 0, lineHeight: 1.4 }}>{icon}</span>
          <div>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600 }}>{lbl}</div>
