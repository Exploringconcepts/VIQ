...
// ── MINI SVG BAR CHART ────────────────────────────────────────────────────────
const MiniBarChart = ({ data, valueKey, color, height = 48 }) => {
  ...
  return (
    <svg viewBox={`0 0 100 ${height}`} style={{ width: "100%", height }} preserveAspectRatio="none">
      {data.map((d, i) => {
        const barH = (d[valueKey] / max) * (height - 6);
        return (
          <g key={i}>
            <rect
              x={i * w + 1} y={height - barH - 1}
              width={w - 2} height={barH}
              rx="2"
              fill={d.yr === 2026 ? `${color}55` : color}
              opacity={0.85}
            />
          </g>
        );
...
const SparkLine = ({ data, valueKey, color }) => {
  ...
  const pts = vals.map((v, i) => {
    const x = (i / (vals.length - 1)) * 98 + 1;
    const y = 38 - ((v - min) / range) * 34;
    return `${x},${y}`;
  }).join(' ');
  return (
    <svg viewBox="0 0 100 42" style={{ width: "100%", height: 42 }} preserveAspectRatio="none">
    ...
    <svg viewBox="0 0 80 80" style={{ width: 80, height: 80 }}>
      ...
      {arcs.map((arc, i) => (
        <circle key={i} cx={cx} cy={cy} r={r} fill="none"
          stroke={arc.color} strokeWidth={stroke - 1}
...
<div style={{
  minHeight: "100vh",
  background: "linear-gradient(160deg, #0b1120 0%, #0f1829 50%, #0a1018 100%)",
...
<style>{`
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  textarea{resize:none;outline:none;border:none;background:transparent;color:#CBD5E1;font-family:inherit;font-size:14px;line-height:1.6;width:100%;}
  textarea::placeholder{color:#334155;}
  .chip:hover{background:rgba(56,189,248,0.12)!important;border-color:rgba(56,189,248,0.4)!important;transform:translateY(-1px);}
  .send-btn:hover{background:#0ea5e9!important;transform:scale(1.05);}
  .send-btn:disabled{opacity:0.35;cursor:not-allowed;}
  .stat-card{transition:transform 0.2s,box-shadow 0.2s;}
  .stat-card:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,0.4)!important;}
  .yr-bar:hover{opacity:1!important;}
  table{border-collapse:collapse;width:100%;}
  th{color:#64748b;font-size:10px;letter-spacing:0.08em;text-transform:uppercase;padding:6px 10px;text-align:left;border-bottom:1px solid #1e293b;}
  td{font-size:12px;padding:6px 10px;border-bottom:1px solid #0f172a;color:#94a3b8;}
...
<div style={{
  ...
  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
    ...
    <div>
      <span style={{ fontFamily: "'Syne', sans-serif", fontSize: 17, fontWeight: 700, color: "#e2e8f0", letterSpacing: "-0.01em" }}>VehicleIQ</span>
      <span style={{ fontSize: 11, color: "#475569", marginLeft: 8, letterSpacing: "0.06em" }}>TOYOTA CAMRY 2014</span>
    </div>
  </div>
  <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
    <span style={{ fontSize: 11, color: "#475569" }}>S. M. Shaarif Anwar</span>
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e", animation: "pulse 2s infinite" }} />
      <span style={{ fontSize: 11, color: "#475569" }}>255 records · Feb 2021 – Mar 2026</span>
    </div>
  </div>
</div>
...
<div style={{
  ...
  {[
    { label: "Total Fuel Spend", value: "$15,174", sub: "5-year total", icon: "💰", color: "#f59e0b", sparkKey: "cost" },
    ...
    <div key={i} className="stat-card" style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <span style={{ fontSize: 10, color: "#475569", letterSpacing: "0.07em", textTransform: "uppercase" }}>{s.label}</span>
        <span style={{ fontSize: 16 }}>{s.icon}</span>
      </div>
      <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 700, color: "#e2e8f0", lineHeight: 1 }}>{s.value}</div>
      <div style={{ fontSize: 10, color: "#475569", marginTop: 4, marginBottom: 8 }}>{s.sub}</div>
      {s.sparkKey && (
        <div style={{ opacity: 0.7 }}>
          <SparkLine data={YEAR_STATS.slice(0,5)} valueKey={s.sparkKey} color={s.color} />
        </div>
      )}
      <div style={{
...
<div style={{
  ...
  <div style={{
    ...
    <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 10 }}>Annual Fuel Spend (CAD)</div>
    <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 60 }}>
      {YEAR_STATS.map((d, i) => {
        const max = Math.max(...YEAR_STATS.map(x => x.cost));
        const h = (d.cost / max) * 52;
        const colors = ["#60a5fa","#38bdf8","#34d399","#f59e0b","#f97316","#a78bfa"];
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <div style={{ fontSize: 9, color: "#475569" }}>${Math.round(d.cost/1000*10)/10}k</div>
            <div className="yr-bar" style={{
              width: "100%", height: h, borderRadius: "3px 3px 0 0",
              background: colors[i], opacity: d.yr === 2026 ? 0.45 : 0.8,
              transition: "opacity 0.2s",
            }} />
            <div style={{ fontSize: 9, color: "#475569" }}>{`'${String(d.yr).slice(2)}`}</div>
          </div>
        );
      })}
    </div>
  ...
  <div style={{
    ...
    <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 6 }}>Efficiency Trend L/100km</div>
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
      <span style={{ fontFamily: "'Syne', sans-serif", fontSize: 20, fontWeight: 700, color: "#34d399" }}>↓ 8.1%</span>
      <span style={{ fontSize: 11, color: "#475569" }}>vs 2022 peak</span>
    </div>
    <SparkLine data={YEAR_STATS.slice(0,5)} valueKey="eff" color="#34d399" />
    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
      {YEAR_STATS.slice(0,5).map(d => (
        <span key={d.yr} style={{ fontSize: 9, color: "#334155" }}>{`'${String(d.yr).slice(2)}`}</span>
      ))}
    </div>
  </div>
  ...
  <div style={{
    ...
    <DonutChart segments={[
      { value: 15174, color: "#38bdf8" },
      { value: 550, color: "#34d399" },
      { value: 102, color: "#f59e0b" },
    ]} />
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 8 }}>Cost Split</div>
      {[
        { label: "Fuel", val: "$15,174", color: "#38bdf8" },
        { label: "Service", val: "~$550", color: "#34d399" },
        { label: "Washes", val: "$102", color: "#f59e0b" },
      ].map((item, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: item.color, flexShrink: 0 }} />
          <span style={{ fontSize: 10, color: "#64748b", flex: 1 }}>{item.label}</span>
          <span style={{ fontSize: 11, color: "#cbd5e1", fontWeight: 600 }}>{item.val}</span>
        </div>
      ))}
    </div>
  </div>
...
<div style={{ flex: 1, display: "flex", overflow: "hidden", maxHeight: "calc(100vh - 280px)" }}>
  ...
  <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
      ...
      {messages.length === 0 && (
        <div style={{ animation: "fadeUp 0.5s ease both" }}>
          <div style={{
            ...
            <div style={{ color: "#64748b", fontSize: 13, lineHeight: 1.6, maxWidth: 500 }}>
              5 years of fuel data, maintenance history, and cost analytics — all queryable in plain English. Try a suggested query below or ask your own.
            </div>
          </div>
      ...
      {messages.map((msg, i) => (
        <div key={i} style={{
          ...
          <div style={{
            ...
            border: msg.role === "user" ? "1px solid rgba(56,189,248,0.3)" : "1px solid rgba(255,255,255,0.06)",
      ...
      {loading && (
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 16, animation: "fadeUp 0.3s ease both" }}>
          <div style={{
            width: 28, height: 28, borderRadius: 8, flexShrink: 0,
            background: "linear-gradient(135deg, #38bdf8, #0369a1)",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <textarea
          ...
          onKeyDown={handleKey}
          placeholder="Ask about fuel costs, efficiency, maintenance, comparisons..."
      ...
      <div style={{ fontSize: 10, color: "#1e293b", textAlign: "center", marginTop: 8, letterSpacing: "0.06em" }}>
        VEHICLEIQ PROOF OF CONCEPT · CLAUDE AI · 2021–2026 DATA
      </div>
    </div>
  </div>
  ...
  <div style={{
    ...
    {/* Year breakdown */}
    <div>
      <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>By Year</div>
      <table>
        <thead>
          <tr>
            <th>Year</th>
            <th style={{ textAlign: "right" }}>Spend</th>
            <th style={{ textAlign: "right" }}>L/100</th>
          </tr>
        </thead>
        <tbody>
          {YEAR_STATS.map(d => (
            <tr key={d.yr}>
              <td>{d.yr === 2026 ? "2026*" : d.yr}</td>
              <td style={{ textAlign: "right", color: d.cost > 3000 ? "#fbbf24" : "#94a3b8" }}>${d.cost.toLocaleString()}</td>
              <td style={{ textAlign: "right", color: d.eff < 9.5 ? "#34d399" : d.eff > 10.5 ? "#f87171" : "#94a3b8" }}>{d.eff}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{ fontSize: 9, color: "#334155", marginTop: 6 }}>* Jan–Mar 2026 only</div>
    </div>

    {/* Maintenance status */}
    <div>
      <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>Maintenance Status</div>
      {[
        { label: "Last Oil Change", val: "Jan 10, 2026", status: "good" },
        { label: "Odometer", val: "~129,329 km", status: "info" },
        ...
        <div key={i} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{
              ...
              background: item.status === "good" ? "#22c55e" : item.status === "warn" ? "#f59e0b" : "#38bdf8",
            }} />
            <span style={{ fontSize: 12, color: "#cbd5e1", fontWeight: 500 }}>{item.val}</span>
          </div>
        </div>
      ))}
    </div>
    ...
    {/* Best vs worst */}
    <div>
      <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>Highlights</div>
      {[
        { label: "Best efficiency", val: "2025 — 9.4 L/100", icon: "✅" },
        { label: "Worst efficiency", val: "2022 — 10.8 L/100", icon: "⚠️" },
        { label: "Cheapest $/L", val: "2026 — $1.14", icon: "💚" },
        { label: "Priciest $/L", val: "2022 — $1.62", icon: "🔴" },
        { label: "Most KM", val: "2024 — 23,126", icon: "🏁" },
      ].map((item, i) => (
        <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 7 }}>
          <span style={{ fontSize: 12, flexShrink: 0 }}>{item.icon}</span>
          <div>
            <div style={{ fontSize: 9, color: "#475569" }}>{item.label}</div>
            <div style={{ fontSize: 11, color: "#94a3b8" }}>{item.val}</div>
          </div>
        </div>
      ))}
    </div>
